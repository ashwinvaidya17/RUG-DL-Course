...
def train_env(env,action_space,state_space,state,total_runs):
    ...
    epochs=1
    runs_completed=0
    update_thresh=10 # exploration threshold
    thresh_flag=False
    t_score=[]
    avg_reward=[]
    step_lst=[]
    ...
    env.close()
    # draw plot
    draw([a for a in range(total_runs)],avg_reward,"Unity Basic","episodes","Avergae reward","blue","ub_avg_reward")
    draw([a for a in range(total_runs)],t_score,"Unitu Basic","episodes","Total reward","red","ub_total_reward")
    draw([a for a in range(total_runs)],step_lst,"Unity Basic","episodes","steps per episode","green","ub_steps_per_episode")
