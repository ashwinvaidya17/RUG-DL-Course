...
def train_env(env,action_space,state_space,state,total_runs):
    ...
    epochs=1
    runs_completed=0
    update_thresh=100 # exploration threshold
    thresh_flag=False
    t_score=[]
    avg_reward=[]
    step_lst=[]
    ...
    rec_step =0 # update the agent if the goal is attained in short compared to the previous episodes
    ...
    for episode in range(total_runs):
        ...
        while not done_flag:
            ...
            curr_step=steps_per_episode
            # commit training
            if done and (curr_step > rec_step):
    ...
    env.close()
    # draw plot
    draw([a for a in range(total_runs)],avg_reward,"Hallway","episodes","Avergae reward","blue","hw_avg_reward")
    draw([a for a in range(total_runs)],t_score,"Hallway","episodes","Total reward","red","hw_total_reward")
    draw([a for a in range(total_runs)],step_lst,"Hallway","episodes","steps per episode","green","hw_steps_per_episode")


if __name__=="__main__":
    arg=argparse.ArgumentParser(description="PPO test")
    arg.add_argument('method',default=None,help="training or testing")
    arg.add_argument('path',default=None,help="path to the UnityEnvironment")
    
    args=arg.parse_args()

    env_unity=UnityEnvironment(args.path)
    env=UnityToGymWrapper(env_unity,use_visual=False)
    state=env.reset()
    ...
    print("Action space:v",action_space)
    print("State space :v",state_space)
