...
def train_env(env,action_space,state_space,state,total_runs):
    ...
    epochs=1
    runs_completed=0
    update_thresh=200 # exploration threshold
    thresh_flag=False
    t_score=[]
    avg_reward=[]
    step_lst=[]
    ...
    env.close()
    # draw plot
    draw([a for a in range(total_runs)],avg_reward,"PushBlock","episodes","Avergae reward","blue","pushblock_avgreward")
    draw([a for a in range(total_runs)],t_score,"PushBlock","episodes","Total reward","red","pushblock_totalreward")
    draw([a for a in range(total_runs)],step_lst,"PushBlock","episodes","steps per episode","green","pushblock_steps_per_episode")
